...
### RollingUpgradePolicy Properties
...
`ManagedDiskType` - (Optional) Specifies the type of managed disk to create. Value you must be either `Standard_LRS`, `StandardSSD_LRS` or `Premium_LRS`. Cannot be used when `VhdContainers` or `Image` is specified.
...
### StorageProfileDataDisk Properties
